...
function refresh(nextURL, DisplayName, refreshInfo) {

    if (parseInt(refreshSec) > 0) {
        refreshSec = parseInt(refreshSec) - 1;
        document.getElementById("refreshIn").innerHTML =
             "Refreshing in: " + refreshMin + " Minutes ," + refreshSec + " Seconds";
        ...
        if (parseInt(refreshSec) == 0) {
            ...
            if (parseInt(refreshMin) == -1) {
                clearTimeout(tim2);
                RunGrahp(nextURL, DisplayName, refreshInfo);
...
function RunGrahp(nextURL, DisplayName, refreshInfo) {
    ...
    $.ajax({
        type: "POST",
        url: "wsPCI/wsPCIOEE.asmx/getGraph_OEE",
