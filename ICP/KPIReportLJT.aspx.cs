...
namespace ICP
{
    public partial class KPIReportLJT : System.Web.UI.Page
    {
        ...
        public MemoryStream GenerateReport_XLS(bool sendMail, int iYear, int iMonth)
        {
            ...
            //addTitulo
            worksheet.Cell(iRow, iCell).Value = "Key Performance Indicator (KPI) Report Lock Joint Tube";
            ...
            if (sendMail)
            {
                dsCategories = da2.ds_KPIReport2("KPICategories", null, null, Convert.ToInt32(iYear), "SWM");
            }
            else
            {
        ...
        private void TB_Exp(ref IXLWorksheet worksheet, ref int iRow, bool sendMail, int iYear, int iMonth)
        {
            ...
            if (sendMail)
            {
                dsCategories = da2.ds_KPIReport2("KPICategories", null, null, Convert.ToInt32(iYear), strCompany);
            }
            else
            {
                dsCategories = (DataSet)ViewState["Categories"];
    ...
    //        MemoryStream memoryStream = report._ms2(iYear, iMonth);
    //        byte[] bytes = memoryStream.ToArray();
    //        bytes = memoryStream.ToArray();
    //        File.WriteAllBytes(path, bytes);
